...
class ArpCache(ControllerModule):
    def __init__(self, cfx_handle, module_config, module_name):
        super(ArpCache, self).__init__(cfx_handle, module_config, module_name)
        # Query CFX to get properties of virtual networks configured by the user
        self.tincanparams = self._cfx_handle.query_param("TincanInterface", "Vnets")
    ...
    def process_cbt(self, cbt):
        ...
        # Process UID-MAC-IP details from other nodes in the network
            ...
            self.register_cbt("Logger", "LOG_DEBUG", "Remote node Unmanaged node details: {0}".format(str(cbt.data)))
            ...
            self.register_cbt("BaseTopologyManager", "UPDATE_MAC_UID_IP_TABLES", UpdateBTMMacUIDTable)
            return
        # Process ARP Packets received
        elif cbt.action == "ARPPacket":
            self.register_cbt("Logger", "debug", "ARP Packet: {0}".format(str(cbt.data)))
            ...
            self.register_cbt("Logger", "debug", "Source MAC:: " + str(srcmac))
            self.register_cbt("Logger", "debug", "Source IP Address::  " + str(srcip))
            self.register_cbt("Logger", "debug", "Destination MAC:: " + str(destmac))
            self.register_cbt("Logger", "debug", "Destination IP Address:: " + str(destip))
        ...
        if op == 1:
            ...
            if destip != self.ipop_vnets_details[interface_name]["ip"]:
                ...
                self.register_cbt("TincanInterface", "DO_INSERT_DATA_PACKET", cbt.data)
            else:
                self.register_cbt("TincanInterface", "DO_INSERT_DATA_PACKET", cbt.data)
                self.register_cbt("BroadcastForwarder", "BroadcastPkt", cbt.data)
        ...
        # Table for setting up routing rules in the Tincan
            ...
            self.register_cbt("BaseTopologyManager", "TINCAN_PACKET", cbt.data)
            ...
            self.register_cbt("BroadcastForwarder", "BroadcastData", sendlocalmacdetails)
