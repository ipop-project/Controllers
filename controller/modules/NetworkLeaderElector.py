...
class NetworkLeaderElector(ControllerModule):
    ...
    def send_PingToLeader(self,stack = []):
        ...
        # source in binding.
        nexthop_uid = self.Binding.get("source")
    ...
    def processCBT(self, cbt):
        ...
        if (cbt.action == "TINCAN_MSG"):
            ...
            if (msg_type == "LSA"):
                if (self.checkState()[0] == "EMPTY"):
                    ...
                    if (msg.get("priority") > self.priority):
                        ...
                        pass
                elif (self.checkState()[0] == "REMOTE"):
                    if (msg.get("priority") > self.Binding.get("priority")):
                        # set leader ping timeout
                        # set retry value
                        # start pinging leader
                        ...
                        pass
                elif (self.checkState()[0] == "PENDING"):
                    if (msg.get("priority") > self.Binding.get("priority")):
                        # set leader ping timeout
                        # set retry value
                        # start pinging leader
                ...
                if (msg.get("target_uid") != self.uid and msg["TTL"]<10):
                    ...
                    if (msg.get("target_uid")== self.uid and self.checkState()[0] == "LOCAL"):
                        # Return a PING_LEADER_ACK message
                        self.send_PingLeaderAck(stack)
            elif (msg_type == "JOIN"):
                if (msg.get("target_uid") != self.uid and msg["TTL"]<10):
                    self.forward_msg(msg)
                elif (msg.get("target_uid")== self.uid and self.checkState()[0] == "LOCAL"):
            ...
            elif (msg_type == "PING_LEADER_ACK" and self.checkState()[0] == "REMOTE"):
                stack = msg.get("stack)
                if (len(stack)==0):
                    # I am the intended recipient
                    # check if response if from current leader
                    # send ping message again
                    if (msg.get("src_uid") == self.Binding.get("leader")):
                        self.send_PingToLeader()
                        self.pingTimerStart = int(time.time())
                        self.ping_retry_counter = 0
                else:
            ...
            elif (msg_type == "JOIN_ACK" and self.checkState()[0] == "REMOTE"):
